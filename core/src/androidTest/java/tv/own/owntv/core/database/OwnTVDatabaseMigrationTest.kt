...
class HanTVDatabaseMigrationTest {
    ...
    fun migrateVersion23ToCurrent_addsCustomCategoryMembers() {
        ...
        try {
            executeSchemaQueries(db23, "tv.own.owntv.core.database.HanTVDatabase/23.json")
    ...
    fun migrateVersion31ToCurrent_addsPlaybackPrefs_andCascadesProfileDelete() {
        ...
        try {
            executeSchemaQueries(db31, "tv.own.owntv.core.database.HanTVDatabase/31.json")
     ...
     * migration now runs HanTVDatabase.healSchema, so opening a drifted v12 database must succeed
    ...
    fun migrateDriftedVersion12ToCurrent_healsDroppedIndexes() {
        ...
        try {
            executeSchemaQueries(db12, "tv.own.owntv.core.database.HanTVDatabase/12.json")
    ...
    fun everyExportedSchemaVersionMigratesToCurrent() {
        MIGRATABLE_START_VERSIONS.forEach { version ->
            ...
            try {
                executeSchemaQueries(old, "tv.own.owntv.core.database.HanTVDatabase/$version.json")
                old.version = version
            } finally {
                old.close()
            }
            ...
            try {
                ...
                HanTVDatabase.EXPECTED_NON_UNIQUE_INDEXES.values.flatten().forEach {
    ...
    fun migration4to6IsANoOpBecauseVersion5WasNeverPublic() {
        assertEquals(4, HanTVDatabase.MIGRATION_4_6.startVersion)
        assertEquals(6, HanTVDatabase.MIGRATION_4_6.endVersion)
        assertEquals(
    ...
    /**
     * D2 — [HanTVDatabase.healSchema] is the last line of defence against the interrupted-import
    ...
    fun healSchemaRestoresEveryGuaranteedIndexAndFtsTable() {
        context.deleteDatabase(DB_NAME)
        val expectedIndexes = HanTVDatabase.EXPECTED_NON_UNIQUE_INDEXES.values.flatten().map(::indexNameOf)
        val expectedFts = HanTVDatabase.EXPECTED_FTS_TABLES.keys

        val current = context.openOrCreateDatabase(DB_NAME, Context.MODE_PRIVATE, null)
        try {
            executeSchemaQueries(current, "tv.own.owntv.core.database.HanTVDatabase/$CURRENT_VERSION.json")
        ...
        try {
            ...
            withConnection(db) { HanTVDatabase.healSchema(it) }
    ...
    private fun identityHashOf(version: Int): String {
        val asset = "tv.own.owntv.core.database.HanTVDatabase/$version.json"
    ...
    private fun bootstrapVersion40Database() {
        ...
        try {
            executeSchemaQueries(db, "tv.own.owntv.core.database.HanTVDatabase/40.json")
            db.execSQL(
    ...
    private fun openWithAllMigrations() = Room.databaseBuilder(context, HanTVDatabase::class.java, DB_NAME)
    ...
    private fun bootstrapVersion2Database() {
        val db = context.openOrCreateDatabase(DB_NAME, Context.MODE_PRIVATE, null)
        try {
            executeSchemaQueries(db, "tv.own.owntv.core.database.HanTVDatabase/2.json")
            seedVersion2Data(db)
            db.version = 2
        } finally {
            db.close()
    ...
    private fun bootstrapVersion7Database() {
        val db = context.openOrCreateDatabase(DB_NAME, Context.MODE_PRIVATE, null)
        try {
            executeSchemaQueries(db, "tv.own.owntv.core.database.HanTVDatabase/7.json")
            seedVersion7Data(db)
            db.version = 7
        } finally {
            db.close()
    ...
    private fun bootstrapVersion29Database() {
        val db = context.openOrCreateDatabase(DB_NAME, Context.MODE_PRIVATE, null)
        try {
            executeSchemaQueries(db, "tv.own.owntv.core.database.HanTVDatabase/29.json")
            db.execSQL(
    ...
    private fun bootstrapVersion3Database() {
        val db = context.openOrCreateDatabase(DB_NAME, Context.MODE_PRIVATE, null)
        try {
            executeSchemaQueries(db, "tv.own.owntv.core.database.HanTVDatabase/3.json")
            seedVersion3Data(db)
            db.version = 3
        } finally {
            db.close()
     ...
     */
    private fun openForAssertions(db: HanTVDatabase): SQLiteConnection {
    ...
    private fun <R> withConnection(db: HanTVDatabase, block: (SQLiteConnection) -> R): R =
    ...
    companion object {
         ...
         */
        private val CURRENT_VERSION = HanTVDatabase.ALL_MIGRATIONS.maxOf { it.endVersion }
