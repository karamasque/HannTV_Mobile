...
class BulkInsertHelper(
    ...
    private suspend fun restoreIndexes(state: BulkIndexState, ftsOnly: Boolean = false) {
        ...
        val canonical = HanTVDatabase.EXPECTED_NON_UNIQUE_INDEXES[state.table].orEmpty()
        db.useWriterConnection { connection ->
            if (!ftsOnly) {
    ...
    companion object {
        ...
        internal val KNOWN_TABLES: Set<String> get() = HanTVDatabase.EXPECTED_NON_UNIQUE_INDEXES.keys
