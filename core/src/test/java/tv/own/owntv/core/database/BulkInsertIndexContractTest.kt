 ...
 * *canonical* set from [HanTVDatabase.EXPECTED_NON_UNIQUE_INDEXES]. A table it is willing to drop
...
class BulkInsertIndexContractTest {
    ...
    fun `bulk-droppable tables are exactly the tables with a canonical index set`() {
        assertEquals(
            HanTVDatabase.EXPECTED_NON_UNIQUE_INDEXES.keys,
    ...
    fun `every canonical entry can actually restore something`() {
        HanTVDatabase.EXPECTED_NON_UNIQUE_INDEXES.forEach { (table, statements) ->
