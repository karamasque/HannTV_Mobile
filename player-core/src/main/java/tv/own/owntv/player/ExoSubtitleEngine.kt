 ...
 * (IPTV panels routinely cap VOD at one). [HanTVPlayer] owns this and mirrors its state into the same
 ...
 * decoder stall, retry budget exhausted), [HanTVPlayer] retries the same item here once ([start] with
...
class ExoSubtitleEngine(
    ...
    /** Hooks back into [HanTVPlayer]'s StateFlows (all fire on the main thread). */
    interface Callbacks {
        ...
        /** Text/image subtitle tracks from the active file — [HanTVPlayer] shows these in the HUD while
    ...
    fun start(
        ...
        sideloadSubs: List<HanTVPlayer.ExternalSub> = emptyList(),
    ...
    fun restoreExternalSubtitles(subs: List<HanTVPlayer.ExternalSub>) {
        val p = player ?: return
        val url = currentUrl ?: return
        var added = false
        subs.forEach { s ->
    ...
    /** Mirrors Settings → Video player → Hardware decoding, pushed in by [HanTVPlayer]. */
     ...
     *  [HanTVPlayer] (F16). Without them a stream that only opens with a custom UA/Referer on mpv would
    ...
    /** This item's Widevine/ClearKey licence details (#115), pushed in by [HanTVPlayer]; null for the
    ...
    /** Called on a ~0.5s tick by [HanTVPlayer] while active, so the HUD scrubber advances. */
    ...
    /** The user's Auto / Stereo only / Surround choice, pushed in by [HanTVPlayer]; read at build time. */
    @Volatile var surroundMode: SurroundMode = SurroundMode.AUTO

    /** Settings → Video player → Auto frame rate, pushed in by [HanTVPlayer]; read at build time. */
     ...
     *  Pushed in by [HanTVPlayer]; applied at build time and in place from [start]. */
     ...
     *
     *  This is ExoPlayer's own software rung, the mirror of mpv's [HanTVPlayer] rescue ladder: without
