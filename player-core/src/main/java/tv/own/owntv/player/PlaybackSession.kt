 ...
 * **MediaSession** (F27). Before this, HanTV neither requested focus nor published a session, so two
 * apps could play over each other and the TV's transport keys / Assistant "pause" never reached the
 * player.
 *
 * It is engine-agnostic on purpose. HanTV plays through mpv *and* two ExoPlayer engines, and which one
 * owns the speaker changes with the content; all three already implement [PlaybackEngine], so this class
 * only ever talks to that interface. `HanTVShell` is the one place that knows which engine is live, so it
